...
class CartScreenProvider extends ChangeNotifier {
  ...
  void getTotalMoneyWithAdd() {
    total = _cartList.fold(
        0,
        (previousValue, element) =>
            previousValue +
            double.parse(element.productPriceWithDiscount ??
  ...
  void getTotalMoneyWithRemove(ProductModel product) {
    String productPrice =
        product.productPriceWithDiscount ?? product.productPrice ?? '0';
...
final cartScreenProvider =
    ChangeNotifierProvider.autoDispose((ref) => CartScreenProvider());
